...
# 偏中心二项分布[-3,-2,-1,0]
def bais_build_centered_binomial_law(k):
    """ Construct the binomial law as a dictionnary
    :param k: (integer)
    :param x: (integer)
    :returns: A dictionnary {x:p_k(x) for x in {-k..k}}
    ...
    D = {}
    for i in range(-k, 0):
        D[i] = 2*centered_binomial_pdf(k, i)
    D[0] = 2 * centered_binomial_pdf(k, 0)
    return D
...
# the short uniform distribution [-k,k-1], 2k elements
def build_uniform_short_law(k):
    """ Construct the uniform law as a dictionnary
    :param k: (integer)
    :param x: (integer)
...
# the edge uniform distribution [-k,0)(0,k], 2k elements
def build_uniform_edge_law(k):
    """ Construct the uniform law as a dictionnary
    :param k: (integer)
    :param x: (integer)
    :returns: A dictionnary {x:p_k(x) for x in {-k..k-1}}
