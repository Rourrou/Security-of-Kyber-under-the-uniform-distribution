...
**************************************************/
void indcpa_keypair_u2k(uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES],
  ...
  // uint8_t prfbuf[UKYBER_PRFBYTES * KYBER_K * 2];
  uint8_t prfbuf[UKYBER_PRFBYTES];
  prf(prfbuf, UKYBER_PRFBYTES, noiseseed, nonce);

  int pos = 0;
  for (i = 0; i < KYBER_K; i++)
    // pos = poly_getnoise_u1k(&skpv.vec[i], prfbuf + (i * UKYBER_PRFBYTES));
    pos += poly_getnoise_u1k(&skpv.vec[i], prfbuf + pos);
  for (i = 0; i < KYBER_K; i++)
    // poly_getnoise_u2k(&e.vec[i], prfbuf + (KYBER_K * UKYBER_PRFBYTES) + (i *
    // UKYBER_PRFBYTES));
    pos += poly_getnoise_u2k(&e.vec[i], prfbuf + pos);
...
*              - uint8_t *sk: pointer to output private key
                              (of length KYBER_INDCPA_SECRETKEYBYTES bytes)
**************************************************/
void indcpa_keypair_u2(uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES],
                       uint8_t sk[KYBER_INDCPA_SECRETKEYBYTES]) {
  unsigned int i;
  uint8_t buf[2 * KYBER_SYMBYTES];
  const uint8_t *publicseed = buf;
  ...
  for (i = 0; i < KYBER_K; i++)
    poly_getnoise_u1(&skpv.vec[i], noiseseed, nonce++);
  for (i = 0; i < KYBER_K; i++)
    poly_getnoise_u2(&e.vec[i], noiseseed, nonce++);
 ...
 **************************************************/
void indcpa_enc_u2k(uint8_t c[KYBER_INDCPA_BYTES],
  ...
  uint8_t prfbuf[UKYBER_PRFBYTES];
  prf(prfbuf, UKYBER_PRFBYTES, coins, nonce);

  int pos = 0;
  for (i = 0; i < KYBER_K; i++)
    pos += poly_getnoise_u1k(sp.vec + i, prfbuf + pos);
  for (i = 0; i < KYBER_K; i++)
    pos += poly_getnoise_u1k(ep.vec + i, prfbuf + pos);
  poly_getnoise_u1k(&epp, prfbuf + pos);
 ...
 *seed (of length KYBER_SYMBYTES) to deterministically generate all randomness
 **************************************************/
void indcpa_enc_u2(uint8_t c[KYBER_INDCPA_BYTES],
                   const uint8_t m[KYBER_INDCPA_MSGBYTES],
                   const uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES],
                   const uint8_t coins[KYBER_SYMBYTES]) {
  unsigned int i;
  ...
  for (i = 0; i < KYBER_K; i++)
    poly_getnoise_u1(sp.vec + i, coins, nonce++);
  for (i = 0; i < KYBER_K; i++)
    poly_getnoise_u1(ep.vec + i, coins, nonce++);
  poly_getnoise_u1(&epp, coins, nonce++);
