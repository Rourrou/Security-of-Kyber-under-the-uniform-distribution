...
int main(void) {
  ...
  */
  ...
  uint8_t prfbuf[UKYBER_PRFBYTES];
  uint8_t noiseseed[KYBER_SYMBYTES];
  randombytes(noiseseed, KYBER_SYMBYTES);
  prf(prfbuf, sizeof(prfbuf), noiseseed, 0);

  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    poly_getnoise_u1k(&ap, prfbuf);
  }
  print_results("poly_getnoise_u1k: ", t, NTESTS);

  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    poly_getnoise_u1(&ap, seed, 0);
  }
  print_results("poly_getnoise_u1: ", t, NTESTS);
  ...
  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    poly_getnoise_u2k(&ap, prfbuf);
  }
  print_results("poly_getnoise_u2k: ", t, NTESTS);

  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    poly_getnoise_u2(&ap, seed, 0);
  }
  print_results("poly_getnoise_u2: ", t, NTESTS);
  ...
  */
  ...
  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    indcpa_keypair_u2k(pk, sk);
  }
  print_results("indcpa_keypair_u1u2k: ", t, NTESTS);

  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    indcpa_keypair_u2(pk, sk);
  }
  print_results("indcpa_keypair_u1u2: ", t, NTESTS);
  ...
  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    indcpa_enc_u2k(ct, key, pk, seed);
  }
  print_results("indcpa_enc_u1k: ", t, NTESTS);

  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    indcpa_enc_u2(ct, key, pk, seed);
  }
  print_results("indcpa_enc_u1: ", t, NTESTS);
  ...
  */
  ...
  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    crypto_kem_keypair_u2k(pk, sk);
  }
  print_results("kyber_keypair_u1u2k: ", t, NTESTS);

  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    crypto_kem_keypair_u2(pk, sk);
  }
  print_results("kyber_keypair_u1u2: ", t, NTESTS);
  ...
  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    crypto_kem_enc_u2k(ct, key, pk);
  }
  print_results("kyber_encaps_u1k: ", t, NTESTS);

  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    crypto_kem_enc_u2(ct, key, pk);
  }
  print_results("kyber_encaps_u1: ", t, NTESTS);
  ...
  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    crypto_kem_dec_u2k(key, ct, sk);
  }
  print_results("kyber_decaps_u1k: ", t, NTESTS);

  for (i = 0; i < NTESTS; i++) {
    t[i] = cpucycles();
    crypto_kem_dec_u2(key, ct, sk);
  }
  print_results("kyber_decaps_u1: ", t, NTESTS);
