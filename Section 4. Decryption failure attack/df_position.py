...
def pos_dfs_ite():
    # A = np.loadtxt("Kyber512_uniform/33322/2000/A.txt", dtype=np.int16)
    A = np.loadtxt("Kyber512_Gaussian/A.txt", dtype=np.int16)
    A_pos = A.copy()
    sign = np.loadtxt("Kyber512_Gaussian/b_1.txt", dtype=np.int16)
    m, n = A.shape
    m = 10
    print("m=", m, "n=", n)
    for i in range(m):
