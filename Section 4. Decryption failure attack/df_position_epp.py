...
def sign_match():
    A_full = np.loadtxt("Kyber512_uniform\A.txt", dtype=np.int16)
    sign_full = np.loadtxt("Kyber512_uniform\sign.txt", dtype=np.int16)
    epp = np.loadtxt("Kyber512_uniform\epp.txt", dtype=np.int16)
    m, n = A_full.shape
    print("m=", m, "n=", n)
...
def epp_dis():
    A_full = np.loadtxt("Kyber512_uniform/33322/3000/Filter/A.txt", dtype=np.int16)
    epp = np.loadtxt("Kyber512_uniform/33322/3000/Filter/epp.txt", dtype=np.int16)
    m, n = A_full.shape
    print("m=", m, "n=", n)
    ...
    ave_epp = sum_epp/m
    print(sum_epp)
    print("the ave_epp is ", ave_epp)

    # 绘制分布直方图
    # plt.hist(epp, bins='auto', alpha=0.7, rwidth=0.85)
    # plt.title('Distribution histogram of |G[i]|')
    # plt.xlabel('Value')
...
def cor_df():
    A = np.loadtxt("Kyber512_uniform\A.txt", dtype=np.int16)
    A_pos = A.copy()
    sign = np.loadtxt("Kyber512_uniform\sign.txt", dtype=np.int16)
    epp = np.loadtxt("Kyber512_uniform\epp.txt", dtype=np.int16)
    print(A)
    m, n = A.shape
    print("m=", m, "n=", n)
    ...
    for i in range(test_num):
        ...
        b_1 = A_pos[rn[0]]
        b_2 = A_pos[rn[1]]
        inn1 = np.dot(b_1, b_2)+epp
        ff.append(inn1)

        r = random.randint(1, 256)
        b_3 = mod_rotation(b_2, r, 256)
        inn2 = np.dot(b_1, b_3)
        fs.append(inn2)
    ...
    # 创建直方图并叠加概率密度曲线
    fig, ax = plt.subplots()
    sns.kdeplot(ff, ax=ax, label='fail-fail', fill=True)
    sns.kdeplot(fs, ax=ax, label='fail-success', fill=True)
    ax.legend()
...
def pos_df2():
    ...
    print("\nexperimental probability of combining 2 ciphertext, successpro %f" %(float(suc_num / test_num)))


# recover the relative position of dfi with dfs
def pos_dfs():
    A = np.loadtxt("Kyber512_uniform\A.txt", dtype=np.int16)
    A_pos = A.copy()
    sign = np.loadtxt("Kyber512_uniform\sign.txt", dtype=np.int16)
    ...
    for test in range(100):
        ...
        for ite in range(100):
            ...
            max_pos_r = 0
            for r in range(256):
                inn = abs(np.dot(b_1, mod_rotation(A_pos[del_pos[ite]], r, 256)))
                if inn > max_inn:
                    max_inn = inn
                    max_pos_r = r
            if max_pos_r == 0:
                suc_num2 += 1
            print(max_inn, max_pos_r)

        print("the", test + 1, "test's success number2 is", suc_num2)
        if suc_num2 >= 90:
            suc_pro += 1
...
def pos_dfs_ite():
    A = np.loadtxt("Kyber512_uniform/33322/3000/Filter/A.txt", dtype=np.int16)
    A_pos = A.copy()
    b = np.loadtxt("Kyber512_uniform/33322/3000/Filter/b.txt", dtype=np.int16)
    b_pos = b.copy()
    epp = np.loadtxt("Kyber512_uniform/33322/3000/Filter/epp.txt", dtype=np.int16)
    epp_pos = epp.copy()
    sign = np.loadtxt("Kyber512_uniform/33322/3000/Filter/sign.txt", dtype=np.int16)
    m, n = A.shape
    qt = 832
    print("m=", m, "n=", n)
    for i in range(m):
    ...
    for test in range(100):
        ...
        for i in range(1, sum_num):
            # b_1 += A_pos[rn[i]]
            failure += A_pos[rn[i]]
            f_1 += A_pos[rn[i]] / np.linalg.norm(A_pos[rn[i]])
        f_1 = f_1 / np.linalg.norm(f_1)
        k = sum_num * qt / np.linalg.norm(failure)
        f_1 = k * f_1
        print(f_1)
        ...
        for df in range(100):
            ...
            for i in range(m):  # For each rotation of vec
                if i not in del_pos:
                    for r in range(256):
                        inn = np.dot(f_1, mod_rotation(A_pos[i], r, 256)) + epp_pos[i]
                        if inn > max_inn:
                            max_inn = inn
                            max_pos_i = i
                            max_pos_r = r
